...
Page {
	...
	GradientListView {
		model: 8 // Up to 8 (A-H)
		delegate: ListNavigtionItem {
			id: distributorDelegate

			readonly property string distributor: String.fromCharCode(65 + index)
			...
			secondaryText: {
				if (!status.valid) {
					...
					//% "Not available"
					return qsTrId("lynxdistributor_not_available")
				...
				if (blown > 0) {
					return blown > 1
						  ...
						  //% "%n fuse(s) blown"
						? qsTrId("lynxdistributor_count_fuses_blown").arg(blown)
						  //% "Fuse blown"
