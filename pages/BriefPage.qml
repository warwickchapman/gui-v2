...
SwipeViewPage {
	...
	function _leftGaugeCount() {
		return (acInputGauge.active ? 1 : 0) + (dcInputGauge.active ? 1 : 0) + (solarYieldGauge.active ? 1 : 0)
	}

	function _rightGaugeCount() {
		return (acLoadGauge.active ? 1 : 0) + (dcLoadGauge.active ? 1 : 0)
	}
